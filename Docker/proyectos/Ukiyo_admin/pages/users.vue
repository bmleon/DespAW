...
const ejecutarCambioDeRol = async () => {
  ...
  // 🔑 Extraemos el token del administrador logueado para autorizar la operación externa
  ...
  try {
    ...
    // Llamada PATCH directa al usuario autenticada con Bearer Token
...
const guardarNuevoUsuario = async () => {
  ...
  try {
    ...
    // 🌟 CORRECCIÓN CRÍTICA: Cambiamos 'username' por 'name' para que la API de NestJS no devuelva un Error 400
    const bodyPayload = {
    ...
    // 🌟 CORRECCIÓN DE REACTIVIDAD: Respetamos 'roleValue' exactamente como vuestro HTML original lo necesita
